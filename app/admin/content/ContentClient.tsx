  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold text-white">Content</h1>
          <p className="text-sm text-stone-400 mt-0.5">Manage page text and media</p>
        </div>
        <button onClick={load} disabled={loading} className="p-2 bg-stone-900 border border-stone-800 rounded-lg hover:bg-stone-800 transition-colors disabled:opacity-50">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(6)].map((_, i) => <div key={i} className="h-20 bg-stone-900 rounded-xl border border-stone-800 animate-pulse" />)}
        </div>
      ) : (
        <div className="space-y-8">
            ...
            <div key={group}>
              ...
              <div className="space-y-3">
                {blocks.filter(b => b.group === group).map(block => (
                  <div key={block.key} className="bg-stone-900 rounded-2xl border border-stone-800 p-4">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      {isDirty(block.key) && (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-500 hover:bg-yellow-400 disabled:bg-stone-700 text-stone-900 text-xs font-semibold rounded-lg transition-colors flex-shrink-0"
                        >
                    ...
                    {block.type === "textarea" ? (
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          onChange={e => setEdits(prev => ({ ...prev, [block.key]: e.target.value }))}
                          className="w-10 h-10 rounded-lg border border-stone-700 cursor-pointer p-0.5 bg-stone-800"
                        />
                        <input
                          value={edits[block.key] ?? ""}
                          onChange={e => setEdits(prev => ({ ...prev, [block.key]: e.target.value }))}
                          className="flex-1 px-3 py-2.5 bg-stone-800 border border-stone-700 rounded-xl text-sm font-mono text-white outline-none focus:border-yellow-500"
                        />
                      </div>
                    ) : (
                      <input
          ...
          {blocks.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-stone-500 text-xs">Run the SQL migration to seed default content blocks</p>
            </div>
          )}
        </div>
      )}
