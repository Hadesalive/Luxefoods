...
export default function CategoriesClient() {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-2">{[...Array(4)].map((_, i) => <div key={i} className="h-16 bg-stone-900 rounded-xl border border-stone-800 animate-pulse" />)}</div>
      ) : (
          ...
          {categories.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-stone-800 border-b border-stone-700">
                <tr>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-stone-400 uppercase tracking-wide">Name</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-stone-400 uppercase tracking-wide hidden sm:table-cell">Slug</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-stone-400 uppercase tracking-wide hidden sm:table-cell">Order</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-stone-400 uppercase tracking-wide">Active</th>
                  <th className="px-4 py-3" />
                </tr>
              </thead>
              <tbody className="divide-y divide-stone-800">
                {categories.map(cat => (
                  <tr key={cat.id} className={cn("hover:bg-stone-800 transition-colors", !cat.is_active && "opacity-50")}>
                    ...
                    <td className="px-4 py-3 text-center text-stone-400 hidden sm:table-cell">{cat.sort_order}</td>
                    <td className="px-4 py-3">
                      <div className="flex justify-center">
                        <button onClick={() => handleToggle(cat)} className={cn("transition-colors", cat.is_active ? "text-yellow-500" : "text-stone-600")}>
                          {cat.is_active
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => openEdit(cat)} className="p-1.5 hover:bg-stone-700 rounded-lg text-stone-400 hover:text-stone-200 transition-colors"><PencilSimple size={14} /></button>
                        {deleteConfirm === cat.id ? (
                          <div className="flex items-center gap-1">
                            <button onClick={() => handleDelete(cat.id)} className="text-[10px] font-semibold text-red-400 hover:underline">Delete</button>
                            <button onClick={() => setDeleteConfirm(null)} className="text-[10px] text-stone-500 hover:underline">Cancel</button>
                          </div>
                        ) : (
                          <button onClick={() => setDeleteConfirm(cat.id)} className="p-1.5 hover:bg-red-500/10 rounded-lg text-stone-600 hover:text-red-400 transition-colors"><Trash size={14} /></button>
                        )}
                      </div>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-stone-900 border border-stone-800 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-stone-800">
              <h2 className="font-bold text-white">{modal.editing ? "Edit Category" : "Add Category"}</h2>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => handleNameChange(e.target.value)} className="w-full px-3 py-2.5 bg-stone-800 border border-stone-700 rounded-xl text-sm text-white outline-none focus:border-yellow-500" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex flex-col justify-end pb-1">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    <span className="text-sm text-stone-200">Active</span>
                  </label>
                </div>
              </div>
            </div>
